...
|---------|---------|---------|---------|---------|---------|
| master_node_addr | string | "" | ip:port | master节点rpc server的监听地址 |  |
...
| nnodes | int32 | 0 | 0～device总数减1 | 每个device的rank id |  |
...
| enable_schedule_overlap | bool | false | true | 是否开启异步调度 |  |
...
| 参数名称 | 类型 | 默认值 | 其他值 | 参数含义 | 其他 |
|---------|---------|---------|---------|---------|---------|
| enable_disagg_pd | bool | false | true | 是否启动PD分离 |  |
...
| 参数名称 | 类型 | 默认值 | 其他值 | 参数含义 | 其他 |
|---------|---------|---------|---------|---------|---------|
| draft_model | string | "" | ip:port | MTP模型所在的路径 |  |
...
| rank_tablefile | string | "" |  | 创建通信域的配置文件 |  |
