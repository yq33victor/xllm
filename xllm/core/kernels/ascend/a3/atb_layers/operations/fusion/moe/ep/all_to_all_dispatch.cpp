...
atb::Status CreateExpertGather(std::map<std::string, uint32_t> &tensorMap, size_t &nodeId, atb::GraphParam &opGraph)
{
...
atb::Status CreateAll2AllDispatchData(std::map<std::string, uint32_t> &tensorMap,
    const AllToAllDispatchParam &param, size_t &nodeId, atb::GraphParam &opGraph)
{
...
atb::Status CreateAllToAllDispatchOperation(const AllToAllDispatchParam &param, atb::Operation **operation)
{
    ...
    uint64_t nodeCount = 5; // 5: total node num
    opGraph.nodes.resize(nodeCount);
    size_t nodeId = 0;

    CHECK_OPERATION_STATUS_RETURN(CreateExpertGather(tensorMap, nodeId, opGraph));
    CHECK_OPERATION_STATUS_RETURN(CreateExpertWeightGather(tensorMap, nodeId, opGraph));
    CHECK_OPERATION_STATUS_RETURN(CreateHiddenGather(tensorMap, nodeId, opGraph));
    CHECK_OPERATION_STATUS_RETURN(CreateAll2AllDispatchData(tensorMap, param, nodeId, opGraph));
    CHECK_OPERATION_STATUS_RETURN(CreateAll2AllDispatchExpert(tensorMap, param, nodeId, opGraph));
