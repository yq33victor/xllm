    ...
    size_t &nodeId, atb::GraphParam &opGraph)
{
    ...
    if (!FLAGS_enable_atb_comm_multiprocess) {
      allToAllParam.commMode = atb::infer::CommMode::COMM_MULTI_THREAD;
    }
...
atb::Status CreateFilteredRecvData(std::map<std::string, uint32_t> &tensorMap, size_t &nodeId, atb::GraphParam &opGraph)
{
...
atb::Status CreateEmptyTensor(std::map<std::string, uint32_t> &tensorMap, size_t &nodeId, atb::GraphParam &opGraph)
{
    atb::Node &mapNode = opGraph.nodes.at(nodeId++);
...
atb::Status CreateIndexAdd(std::map<std::string, uint32_t> &tensorMap, size_t &nodeId, atb::GraphParam &opGraph)
{
    atb::Node &indexAddNode = opGraph.nodes.at(nodeId++);
...
atb::Status CreateAllToAllCollectOperation(const AllToAllCollectParam &param, atb::Operation **operation)
{
    ...
    uint64_t nodeCount = 4; // 4: 初始化节点个数
    if (param.hasMoeTp) {
        nodeCount += 2; // 2: Moe TP 新增2个节点
    }
    opGraph.nodes.resize(nodeCount);
    size_t nodeId = 0;
    if (param.hasMoeTp) {
        CHECK_OPERATION_STATUS_RETURN(CreateReduceScatterData(tensorMap, param, nodeId, opGraph));
    }
    CHECK_OPERATION_STATUS_RETURN(CreateAll2AllCollectData(tensorMap, param, nodeId, opGraph));
    CHECK_OPERATION_STATUS_RETURN(CreateFilteredRecvData(tensorMap, nodeId, opGraph));
    CHECK_OPERATION_STATUS_RETURN(CreateEmptyTensor(tensorMap, nodeId, opGraph));
    CHECK_OPERATION_STATUS_RETURN(CreateIndexAdd(tensorMap, nodeId, opGraph));
