...
EventManagerStatus EventManager::CreateAndPushEvent(aclrtEvent &event, const std::string &pipeKey)
{
    ...
    ATB_SPEED_LOG_DEBUG("PushEvent: event = " << event
                        ...
                        << ", current eventCount = " << eventCount_);
...
{
    if (eventQueues_.find(pipeKey) == eventQueues_.end() || eventQueues_[pipeKey].empty()) {
        ATB_SPEED_LOG_ERROR("PopEvent: No event available for pipeKey " << pipeKey \
                            << ", current eventCount = " << eventCount_);
                                        ...
                                        const std::string &pipeKey)
{
    ...
    if (eventAction == EventAction::PUSH) {
        ...
        CHECK_EM_STATUS_RETURN_AND_LOG_IF_ERROR(ret, EM_POP_EVENT_FAILED,
            eventTypeStr + ": PopEvent failed with error");
