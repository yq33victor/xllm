...
int64_t DecoderModel::BuildGraph()
{
    ...
    std::map<uint32_t, uint32_t> tensorIndexMap = {};
    if (this->param.enableDap) {
        tensorIndexMap = CopyMapWithSuffix(this->inTensorMap);
    }
    std::stringstream ss;
    ...
    if (this->param.enableDap) {
        for (auto tensor = tensorIndexMap.cbegin(); tensor != tensorIndexMap.cend(); ++tensor) {
            ss << "tensor src index: " << tensor->first << ", tensor dst index: " << tensor->second << std::endl;
        }
    }
    ...
    std::map<uint32_t, uint32_t> internalTensorIndexMap = {};
    if (this->param.enableDap) {
        internalTensorIndexMap = CopyMapWithSuffix(this->internalTensorMap);
    }
    ss.str("");
    ss.clear();
    ...
    if (this->param.enableDap) {
        for (auto tensor = internalTensorIndexMap.cbegin(); tensor != internalTensorIndexMap.cend(); ++tensor) {
            ss << "tensor src index: " << tensor->first << ", tensor dst index: " << tensor->second << std::endl;
        }
    }
    ...
    // Construct precederToSuccessorTensorMap for Dap
    if (this->param.enableDap) {
        for (auto pair = tensorIndexMap.cbegin(); pair != tensorIndexMap.cend(); ++pair) {
            this->precederToSuccessorTensorMap[&graph_.inTensors.at(pair->first)] = &graph_.inTensors.at(pair->second);
        }
        for (auto pair = internalTensorIndexMap.cbegin(); pair != internalTensorIndexMap.cend(); ++pair) {
            this->precederToSuccessorTensorMap[&graph_.internalTensors.at(pair->first)] = \
                &graph_.internalTensors.at(pair->second);
        }
    }

    // 准备outTensor
    uint32_t numOfOutTensors = 1;
    if (this->param.enableDap) {
        numOfOutTensors++;
    }
    graph_.outTensors.resize(numOfOutTensors);
    // Update precederToSuccessorTensorMap for Dap
    if (this->param.enableDap) {
        this->precederToSuccessorTensorMap[&graph_.outTensors.at(0)] = &graph_.outTensors.at(1);
    }
...
atb::Status DecoderModel::AddOperationToGraph()
{
    ...
    if (this->param.enableDap) {
        CHECK_OPERATION_STATUS_RETURN(
            atb_speed::EventManager::GetInstance().WaitEvent(op, atb_speed::EventAction::PUSH, common::COMPUTE_EVENT));
        ...
        ss.str("");
        ss << "[Events] [SUCCESSOR] [PUSH] [WAIT] [COMPUTE]";
    ...
    if (param.enableDap) {
        CHECK_OPERATION_STATUS_RETURN(
            atb_speed::EventManager::GetInstance().RecordEvent(op, atb_speed::EventAction::POP, common::COMPUTE_EVENT));
        ...
        ATB_SPEED_LOG_DEBUG("[Events] [PRECEDER] [POP] [RECORD] [COMPUTE]");

        CHECK_OPERATION_STATUS_RETURN(
            atb_speed::EventManager::GetInstance().WaitEvent(op, atb_speed::EventAction::POP, common::COMM_EVENT));
        ...
        ATB_SPEED_LOG_DEBUG("[Events] [PRECEDER] [POP] [WAIT] [COMM]");
    ...
    GetSingleton<common::CommOpCounter>().Reset();
...
atb::Status DecoderModel::AddLayer()
{
    ...
    for (uint32_t layerId = 0; layerId < numHiddenLayers; ++layerId) {
        if (param.enableDap && (layerId % 2 == 0)) {  // 2: even layer
            GetSingleton<common::DapManager>().SetRole(common::DapRole::SUCCESSOR);
        }
        ATB_SPEED_LOG_DEBUG("Add layer start: " << GetSingleton<common::DapManager>().GetRole());
...
void DecoderModel::ParseDapParam(nlohmann::json &paramJson)
{
    ...
    this->tokenOffsetForDap.clear();
    for (auto item : paramJson["tokenOffsetForDap"]) {
        this->tokenOffsetForDap.push_back(item.get<int>());
        ATB_SPEED_LOG_DEBUG("tokenOffsetForDap value: " << item);
    }

    this->seqLenForDap.clear();
    for (auto item : paramJson["seqLenForDap"]) {
        this->seqLenForDap.push_back(item.get<int>());
        ATB_SPEED_LOG_DEBUG("seqLenForDap value: " << item);
    }

    this->qLenForDap.clear();
    for (auto item : paramJson["qLenForDap"]) {
        this->qLenForDap.push_back(item.get<int>());
        ATB_SPEED_LOG_DEBUG("qLenForDap value: " << item);
    }
}
...
atb::Status DecoderModel::BindParamHostTensor(uint32_t nodeId)
{
    ...
    if (param.enableDap) {
        ...
        std::string suffix = GetSingleton<common::DapManager>().GetSuccessorSuffix();
        // Bind dap tensor
        tensorIdx = atb_speed::common::GetTensorIdx(this->inTensorMap, "token_offset" + suffix);
        if (tensorIdx != UINT32_MAX) {
            graph_.inTensors.at(tensorIdx).hostData = tokenOffsetForDap.data();
        }

        tensorIdx = atb_speed::common::GetTensorIdx(this->inTensorMap, "seq_len" + suffix);
        if (tensorIdx != UINT32_MAX) {
            graph_.inTensors.at(tensorIdx).hostData = seqLenForDap.data();
        }

        tensorIdx = atb_speed::common::GetTensorIdx(this->inTensorMap, "q_len" + suffix);
        if (tensorIdx != UINT32_MAX) {
            graph_.inTensors.at(tensorIdx).hostData = this->qLen.data();
        }
