    ...
    atb::GraphParam &opGraph, std::map<std::string, uint32_t> &tensorMap)
{
    ...
    if (param.enableAclnnRmsNorm) {
        qNormNode.operation = new atb_speed::common::RmsNormOperation("qRmsNorm", qkNormParam.normParam.epsilon);
    } else {
        CHECK_OPERATION_STATUS_RETURN(atb::CreateOperation(qkNormParam, &qNormNode.operation));
    }

    if (param.enableAclnnRmsNorm) {
        kNormNode.operation = new atb_speed::common::RmsNormOperation("kRmsNorm", qkNormParam.normParam.epsilon);
    } else {
